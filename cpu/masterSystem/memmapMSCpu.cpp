#include "memmapMSCpu.h"
#include<iostream>
#include<fstream>

memmapMSCpu::memmapMSCpu(std::shared_ptr<config> cfg) {
...
uint8_t& memmapMSCpu::readByte(uint32_t addr) {
    return map(addr);
}

uint16_t& memmapMSCpu::readWord(uint32_t addr) {
    return reinterpret_cast<uint16_t&>(map(addr));
}

uint32_t& memmapMSCpu::readLong(uint32_t addr) {
    return reinterpret_cast<uint32_t&>(map(addr));
}

void memmapMSCpu::writeByte(uint32_t addr, uint8_t val) {}
void memmapMSCpu::writeWord(uint32_t addr, uint16_t val) {}
void memmapMSCpu::writeLong(uint32_t addr, uint32_t val) {}

uint8_t& memmapMSCpu::map(uint32_t addr) {
    if(addr < 0xC000) {
        return rom[addr];
    }
    else if(addr < 0xE000) {
